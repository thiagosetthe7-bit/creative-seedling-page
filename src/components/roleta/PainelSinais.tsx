        ...
        {ultimos.map((s) => (
          <li key={s.id} className="border-l-4 px-3 py-2 text-xs" style={{ borderLeftColor: s.colorCode }}>
            ...
            <div className="mt-0.5 text-muted-foreground">
              {s.bip === "timer" ? "BT" : "BR"} · entrada: {s.mainAction} · resultado: {s.auditNumero ?? "—"} · {s.auditResult === "NO_BET" ? "⏸ SEM APOSTA" : s.auditResult === "GREEN" ? `✅ GREEN (${s.auditTimestamp ? new Date(s.auditTimestamp).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}) : "--:--"})` : s.auditResult === "RED" ? `❌ RED (${s.auditTimestamp ? new Date(s.auditTimestamp).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}) : "--:--"})` : s.auditResult === "PARTIAL" ? `🟡 PARTIAL (${s.auditTimestamp ? new Date(s.auditTimestamp).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}) : "--:--"})` : "⏳ AGUARDANDO RESULTADO"}
            </div>
          </li>
        ))}
      </ul>
